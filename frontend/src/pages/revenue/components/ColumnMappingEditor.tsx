...
export default function ColumnMappingEditor({
  ...
  const columns: ColumnsType<{ header: string }> = [
    { title: 'Excel Column', dataIndex: 'header', key: 'header' },
    {
...
export function mappingsToLines(
  ...
  return headers
    .filter((h) => mappings[h])
    .map((h) => ({ excelColumnName: h, systemAttribute: mappings[h] }));
}
